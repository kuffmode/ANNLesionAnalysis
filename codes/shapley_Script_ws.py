...
_, \
_, \
_ = exto.network_structure(config, optimized_genome)
...
gpto.compute_shapley(weight_swapped_genome,
                     scores_table,
                     shapley_table,
                     ae_file,
                     config,
...
shapley_sorted.to_csv(f'SV_SW{n_samples}.csv', index=False)

with open(f'SV_SW{n_samples}.pkl', 'wb') as output:
    pickle.dump(shapley_sorted, output, 1)

with open(f'SV_SW{n_samples}_scores_table.pkl', 'wb') as output:
